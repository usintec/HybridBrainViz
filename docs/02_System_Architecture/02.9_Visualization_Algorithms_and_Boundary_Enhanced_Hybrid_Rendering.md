...
B(v) =
\begin{cases}
1: & \text{if any neighboring voxel belongs to a different class} \ 0: & \text{otherwise}
\end{cases}
$$

Using a 6-connected neighborhood:

N_6(v) = {v_x+1, v_x-1, v_y+1, v_y-1, v_z+1, v_z-1}

$$
N_6(v) = {v_{x+},v_{x-},v_{y+},v_{y-},v_{z+},v_{z-}}
$$
...
D_B(v) = exp(-d(v, B)^2 / (2σ_B^2))

$$
D_B(v) = \exp \left( -\frac{d(v,B)^2}{2\sigma_B^2} \right)
$$
where:
